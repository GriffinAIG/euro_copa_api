...
export class SysConfigsController {
  ...
  @Roles(UserRoles.SUPPER, UserRoles.ADMIN, UserRoles.SYSTEM_CONFIG_UPDATE)
  async updateList(
    ...
    if (updateListSysConfigsDto?.sysConfigs?.length > 0) {
      for (const sysConfig of updateListSysConfigsDto.sysConfigs) {
        await this.sysConfigsService.updateOrCreate(
          sysConfig.id,
          sysConfig,
          res.user
        );
  ...
  // @Get("tickets-info")
  // @ApiOperation({
  //   description: "game-info (ticket1, ticket2, ticket3)",
  // })
  // @ApiOkResponse({
  //   type: Response<SysConfig[]>,
  // })
  // async getTicketInfo(): Promise<any> {
  //   return this.sysConfigsService.getTicketInfo();
  // }

  // @Get("award-congul")
  // @ApiOperation({
  //   description: "game-info (wheel1, wheel2)",
  // })
  // @ApiOkResponse({
  //   type: Response<SysConfig[]>,
  // })
  // async getAwardCongul(): Promise<any> {
  //   return this.sysConfigsService.getAwardCongul();
  // }
  ...
  async update(
    ...
    return this.sysConfigsService.updateOrCreate(id, SysConfigsDto, res.user);
  }
